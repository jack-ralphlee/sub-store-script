...
// 清理外部订阅专属参数，保留 icon-url、hidden 等通用参数
const allowedSettings = settings.filter(s => 
  !s.startsWith('policy-path') && 
  !s.startsWith('update-interval') && 
  !s.startsWith('no-alert') && 
  !s.startsWith('include-all-proxies')
)

// 组装新策略组并写入（此处不再加入 nodeNames，从而不显示自建节点名称）
const allNodes = [...new Set([...originalNodes])]
