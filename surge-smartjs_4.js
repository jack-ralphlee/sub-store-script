...
// 自建节点名称匹配正则（在此修改你自建节点包含的关键字）
const selfBuiltKeywords = /自建|Home|VPS|Direct|直连/i

// 只将非自建节点放入策略组
const airportNodesOnly = nodeNames.filter(name => !selfBuiltKeywords.test(name))

// 组装新策略组并写入
const allNodes = [...new Set([...airportNodesOnly, ...originalNodes])]
